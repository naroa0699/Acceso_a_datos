...
public class Dao {
    ...
    public void crearTablaCompanies(Connection connection) {
        ...
        try (Statement stmt = connection.createStatement()) {
            stmt.execute(sql);
            System.out.println("Tabla COMPANIES creada o ya existente.");
        } catch (SQLException e) {
            System.err.println("Error al crear la tabla COMPANIES.");
            e.printStackTrace();
        }
    }

    public void insertarCompaniesBatch(Connection connection, List<Company> companies) {
        ...
        try {
            ...
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ...
                ps.executeBatch();
                connection.commit();
                System.out.println("Batch de compañías insertado correctamente.");

            }

        } catch (SQLException e) {
            System.err.println("Error en la inserción batch. Haciendo rollback...");
            try { connection.rollback(); } catch (SQLException ex) { ex.printStackTrace(); }
            e.printStackTrace();
        } finally {
            try { connection.setAutoCommit(auto); } catch (SQLException e) { e.printStackTrace(); }
        }
    }
}
