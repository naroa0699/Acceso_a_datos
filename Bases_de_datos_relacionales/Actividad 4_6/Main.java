...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // Lista de DNIs que queremos consultar
            List<String> dnisParaMostrar = Arrays.asList("78901234X", "89012345E", "56789012B", "09876543K");

// Llamada al método de Dao
            System.out.println("\n--- Mostrando clientes por DNI ---");
            dao.mostrarClientesPorDNI(dnisParaMostrar);
...
//            print.imprimirRegistros(connection, CATALOGO, NOMBRE_TABLA);
            ...
            dao.crearTablaCompanies(connection);

// Insertar lista de compañías en batch
            List<Company> listaCompanies = Arrays.asList(
                    new Company("12345678A", "TechCorp", "Tecnología"),
                    new Company("87654321B", "Foodies", "Alimentación"),
                    new Company("11223344C", "ConstructPlus", "Construcción")
            );

            connection.setAutoCommit(false);
            try {
                int[] resultados = dao.insertarCompaniesBatch(connection, listaCompanies);
                connection.commit();
                System.out.println("Inserción batch realizada con éxito.");
            } catch (SQLException ex) {
                connection.rollback();
                System.err.println("Error en batch. Rollback realizado.");
            }
            connection.setAutoCommit(true);
