...
public class ImprimirResultados {
    ...
    public void imprimirEmpleadosInverso(Connection conn) throws SQLException {
        // 1. Consulta original (no se modifica)
        String sql = "SELECT CONCAT(first_name, ' ', last_name) AS name FROM employees;";

        // 2. Crear lista temporal para guardar los resultados
        List<String> nombres = new ArrayList<>();

        // 3. Ejecutar la consulta y llenar la lista
        try (PreparedStatement pstmt = conn.prepareStatement(sql);
             ResultSet rs = pstmt.executeQuery()) {
        ...
        // 4. Invertir el orden de los nombres
        Collections.reverse(nombres);

        // 5. Mostrar resultados en consola
        System.out.println("📋 Lista de empleados (del último al primero):");
        System.out.println("---------------------------------------------");
